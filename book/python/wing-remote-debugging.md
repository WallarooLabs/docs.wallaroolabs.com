...
```bash
cd ~/wallaroo-tutorial/wallaroo-0.6.0/examples/python/reverse
```
...
Now that our project is setup, let's open the `~/wallaroo-tutorial/wallaroo-0.6.0/examples/python/reverse/wingdbstub.py` file via Wing's `File -> Open Remote File`.
...
We'll also open the `~/wallaroo-tutorial/wallaroo-0.6.0/examples/python/reverse/reverse.py` file via Wing's `File -> Open Remote File`
...
Now that we're setup, let's run our `reverse` example. You can find the instructions [here](https://github.com/WallarooLabs/wallaroo/tree/0.6.0/examples/python/reverse/README.md).
